const PluginEverIconPicker = function(option) {
	"use strict";
	var allIcons = {
		"font-awesome":{
			"regular":{
	...
	var plugineverWrap = '' +
		'<div class="aim-modal aim-open">' +
			'<div class="aim-modal--content">' +
				'<div class="aim-modal--header">' +
					'<div class="aim-modal--header-logo-area">' +
						'<span class="aim-modal--header-logo-title">Plugin Ever Icon Picker</span>' +
					'</div>' +
	...
	var plugineverDomEle = createDomEle(plugineverWrap);
	var sidebarTabs = plugineverDomEle.querySelector('.aim-modal--sidebar-tabs');
	var previewWrap = plugineverDomEle.querySelector('.aim-modal--icon-preview');
	var searchInput = plugineverDomEle.querySelector('.aim-modal--icon-search input');
	...
	document.querySelector(defaultOpt.onClick).addEventListener('click', function (e) {
		if( null == selector.querySelector('.aim-modal') ) {
			//push pluginever dom to selector
			selector.appendChild( plugineverDomEle );
		}else{
			//Icon library open if dom element exist
			plugineverDomEle.classList.remove('aim-close');
			plugineverDomEle.classList.add('aim-open');
		}
		document.querySelector('.aim-open .aim-modal--icon-search input').focus();
		//Icon library close by clicking close button
		plugineverDomEle.querySelector('.aim-modal--header-close-btn').addEventListener('click', function (e) {
			plugineverDomEle.classList.add('aim-close');
			plugineverDomEle.classList.remove('aim-open');
		});
		...
		document.querySelectorAll('.aim-icon-item').forEach(function (item, key) {
			item.addEventListener('click', function (e) {
				iconWrap.forEach(function (item, key) {
					item.classList.remove('pluginever-selected');
				});
				// e.currentTarget.classList.toggle('pluginever-selected');
				e.currentTarget.classList.add('pluginever-selected');
			});
		});

		//Insert button
		plugineverDomEle.querySelector('.aim-insert-icon-button').addEventListener('click', function (e) {
			e.preventDefault();
			var selected = document.querySelector('.pluginever-selected');
			if (null !== selected) {
	...
	document.querySelector(defaultOpt.removeIcon).addEventListener('click', function (e) {
		document.querySelector(option.iconPlaceholder+' i').classList.value = 'fas fa-circle';
	...
	function sidebarFilterFunc(filterItems, filterText ) {
		filterIcon = Object.entries(filterItems).filter(function (item, index) {
			return 'all' === filterText || filterText === item[1].dataset['libraryId'];
		});
		iconItemPush(filterIcon);
	}

	function clickHandlerFunc(e) {
		if (!e.currentTarget.classList.contains('pluginever-active')) {
			sideBarBtn.forEach(function (item, key) {
				item.classList.remove('pluginever-active');
			});
			e.currentTarget.classList.add('pluginever-active')
		}
	...
	function sideBarListMarkup(sideBarList) {
		...
		sideBarList.forEach(function (item, index) {
			if ('all' !== item['library-id']) {
				...
				markup += '<div class="aim-modal--sidebar-tab-item pluginever-active" data-library-id="'+item['library-id']+'"><i class="'+item['list-icon']+'"></i>'+item['title']+'</div>';
			}
		});
...
export default PluginEverIconPicker;
