...
imgLiquid.bgs_Available = !1, imgLiquid.bgs_CheckRunned = !1, imgLiquid.injectCss = ".imgLiquid img {visibility:hidden}", function (l) {
	l.fn.extend({
		imgLiquid: function (i) {
			this.defaults = {
				...
				if (!imgLiquid.bgs_CheckRunned) {
					...
					var t = l('<span style="background-size:cover" />');
					l("body").append(t), function () {
						var i = t[0];
			...
			var r = this;
			return this.options = i, this.settings = l.extend({}, this.defaults, this.options), this.settings.onStart && this.settings.onStart(), this.each(function (u) {
				var h = r.settings, f = l(this), g = l("img:first", f);

				function o() {
					(h.responsive || g.data("imgLiquid_oldProcessed")) && g.data("imgLiquid_settings") && (h = g.data("imgLiquid_settings"), f.actualSize = f.get(0).offsetWidth + f.get(0).offsetHeight / 1e4, f.sizeOld && f.actualSize !== f.sizeOld && n(), f.sizeOld = f.actualSize, setTimeout(o, h.responsiveCheckTime))
				}

				function s() {
					g.data("imgLiquid_error", !0), f.addClass("imgLiquid_error"), h.onItemError && h.onItemError(u, f, g), v()
				}

				function n() {
					var i, e, t, o, s, n, r, l, a = 0, d = 0, c = f.width(), p = f.height();
					g.data("owidth") === undefined && g.data("owidth", g[0].width), g.data("oheight") === undefined && g.data("oheight", g[0].height), !h.fill && g.data("owidth") <= c && g.data("oheight") <= p ? (e = i = "auto", t = g.data("owidth"), o = g.data("oheight")) : h.fill === c / p >= g.data("owidth") / g.data("oheight") ? (i = "100%", e = "auto", t = Math.floor(c), o = Math.floor(c * (g.data("oheight") / g.data("owidth")))) : (i = "auto", e = "100%", t = Math.floor(p * (g.data("owidth") / g.data("oheight"))), o = Math.floor(p)), r = c - t, "left" === (s = h.horizontalAlign.toLowerCase()) && (d = 0), "center" === s && (d = .5 * r), "right" === s && (d = r), -1 !== s.indexOf("%") && 0 < (s = parseInt(s.replace("%", ""), 10)) && (d = r * s * .01), l = p - o, "top" === (n = h.verticalAlign.toLowerCase()) && (a = 0), "center" === n && (a = .5 * l), "bottom" === n && (a = l), -1 !== n.indexOf("%") && 0 < (n = parseInt(n.replace("%", ""), 10)) && (a = l * n * .01), h.hardPixels && (i = t, e = o), g.css({
						width: i,
						height: e,
						"margin-left": Math.floor(d),
						"margin-top": Math.floor(a)
					}), g.data("imgLiquid_oldProcessed") || (g.fadeTo(h.fadeInTime, 1), g.data("imgLiquid_oldProcessed", !0), h.removeBoxBackground && f.css("background-image", "none"), f.addClass("imgLiquid_nobgSize"), f.addClass("imgLiquid_ready")), h.onItemFinish && h.onItemFinish(u, f, g), v()
				}

				function v() {
					u === r.length - 1 && r.settings.onFinish && r.settings.onFinish()
				}

				g.length ? (g.data("imgLiquid_settings") ? (f.removeClass("imgLiquid_error").removeClass("imgLiquid_ready"), h = l.extend({}, g.data("imgLiquid_settings"), r.options)) : h = l.extend({}, r.settings, function () {
					var i = {};
					if (r.settings.useDataHtmlAttr) {
						var e = f.attr("data-imgLiquid-fill"), t = f.attr("data-imgLiquid-horizontalAlign"),
							o = f.attr("data-imgLiquid-verticalAlign");
						"true" !== e && "false" !== e || (i.fill = Boolean("true" === e)), t === undefined || "left" !== t && "center" !== t && "right" !== t && -1 === t.indexOf("%") || (i.horizontalAlign = t), o === undefined || "top" !== o && "bottom" !== o && "center" !== o && -1 === o.indexOf("%") || (i.verticalAlign = o)
					}
					imgLiquid.isIE && r.settings.ieFadeInDisabled && (i.fadeInTime = 0);
					return i
				}()), g.data("imgLiquid_settings", h), h.onItemStart && h.onItemStart(u, f, g), imgLiquid.bgs_Available && h.useBackgroundSize ? function () {
					if (-1 === f.css("background-image").indexOf(encodeURI(g.attr("src")))) {
						var i = g.data("src"), e = g.attr("src");
						i !== undefined && (e = g.data("src")), f.css({"background-image": 'url("' + encodeURI(e) + '")'})
					}
					f.css({
						"background-size": !h.fill && g[0].width <= f.width() && g[0].height <= f.height() ? "auto" : h.fill ? "cover" : "contain",
						...
						width: "100%",
						height: "100%"
					}), l("img", f).css({display: "none"}), h.onItemFinish && h.onItemFinish(u, f, g);
					f.addClass("imgLiquid_bgSize"), f.addClass("imgLiquid_ready"), v()
				}() : function e() {
					if (g.data("oldSrc") && g.data("oldSrc") !== g.attr("src")) {
						var t = g.clone().removeAttr("style");
						return t.data("imgLiquid_settings", g.data("imgLiquid_settings")), g.parent().prepend(t), g.remove(), (g = t)[0].width = 0, void setTimeout(e, 10)
					}
					if (g.data("imgLiquid_oldProcessed")) return void n();
					g.data("imgLiquid_oldProcessed", !1);
					g.data("oldSrc", g.attr("src"));
					l("img:not(:first)", f).css("display", "none");
					...
					g[0].onerror = s;
					!function i() {
						if (g.data("imgLiquid_error") || g.data("imgLiquid_loaded") || g.data("imgLiquid_oldProcessed")) return;
						f.is(":visible") && g[0].complete && 0 < g[0].width && 0 < g[0].height ? (g.data("imgLiquid_loaded", !0), setTimeout(n, u * h.delay)) : setTimeout(i, h.timecheckvisibility)
					}();
					o()
				}()) : s()
			})
	...
	var i = imgLiquid.injectCss, e = document.getElementsByTagName("head")[0], t = document.createElement("style");
	t.type = "text/css", t.styleSheet ? t.styleSheet.cssText = i : t.appendChild(document.createTextNode(i)), e.appendChild(t)
}(), function (i) {
	"use strict";
	...
	(s = 0, r = function (i, e) {
		...
		t.$slides.add(t.$slideTrack.find(".slick-cloned")).attr({
			...
			d(this).attr({
				...
				tabindex: -1
			}), -1 !== e && d(this).attr({"aria-describedby": "slick-slide-control" + t.instanceUid + e})
		...
		var i = this;
		!0 === i.options.dots && (d("li", i.$dots).on("click.slick", {message: "index"}, i.changeSlide), !0 === i.options.accessibility && i.$dots.on("keydown.slick", i.keyHandler)), !0 === i.options.dots && !0 === i.options.pauseOnDotsHover && d("li", i.$dots).on("mouseenter.slick", d.proxy(i.interrupt, i, !0)).on("mouseleave.slick", d.proxy(i.interrupt, i, !1))
		...
		var e, t, o, n = this;
		if (!0 === n.options.centerMode ? !0 === n.options.infinite ? o = (t = n.currentSlide + (n.options.slidesToShow / 2 + 1)) + n.options.slidesToShow + 2 : (t = Math.max(0, n.currentSlide - (n.options.slidesToShow / 2 + 1)), o = n.options.slidesToShow / 2 + 1 + 2 + n.currentSlide) : (t = n.options.infinite ? n.options.slidesToShow + n.currentSlide : n.currentSlide, o = Math.ceil(t + n.options.slidesToShow), !0 === n.options.fade && (0 < t && t--, o <= n.slideCount && o++)), e = n.$slider.find(".slick-slide").slice(t, o), "anticipated" === n.options.lazyLoad) for (var s = t - 1, r = o, l = n.$slider.find(".slick-slide"), a = 0; a < n.options.slidesToScroll; a++) s < 0 && (s = n.slideCount - 1), e = (e = e.add(l.eq(s))).add(l.eq(r)), s--, r++;
		...
		var e, t, o = this;
		t = o.slideCount - o.options.slidesToShow, !o.options.infinite && o.currentSlide > t && (o.currentSlide = t), o.slideCount <= o.options.slidesToShow && (o.currentSlide = 0), e = o.currentSlide, o.destroy(!0), d.extend(o, o.initials, {currentSlide: e}), o.init(), i || o.changeSlide({
			data: {
				message: "index",
				index: e
			}
		...
		if ("object" === d.type(arguments[0]) ? (t = arguments[0], r = arguments[1], s = "multiple") : "string" === d.type(arguments[0]) && (t = arguments[0], o = arguments[1], r = arguments[2], "responsive" === arguments[0] && "array" === d.type(arguments[1]) ? s = "responsive" : void 0 !== arguments[1] && (s = "single")), "single" === s) n.options[t] = o; else if ("multiple" === s) d.each(t, function (i, e) {
			n.options[i] = e
...
}), window.WC_Category_Showcase = function (i, e, s, t) {
	"use strict";
	var o = {
		init: function () {
			var i = s(".woo-cs-slider");
			i.on("init", function () {
				if (!function () {
					var i = ["android", "webos", "iphone", "ipad", "blackberry"];
					for (var e in i) if (navigator.userAgent.match("/" + i[e] + "/i")) return !0;
					return !1
				}()) {
					var o = s(".woo-cs");
					s.each(o, function (i, e) {
						var t = s(e).find(".woo-cs-right-block>.row").height();
						s(e).find(".woo-cs-left-block").height() !== t && (s(o).find(".slick-track").css("max-height", t + "px"), s(o).find(".woo-cs-slider-block img").css("width", "auto"))
					})
				}
				s(this).closest(".woo-cs").addClass("initialized")
			}), i.each(function () {
				var i = s(this).data("slider-config"), e = i.autoplay, t = i.infinite, o = i.speed;
				s(this).slick({autoplay: e, dots: !1, infinite: t, speed: o})
			}), s("body").find('input[name="optimole_active"]').val() || s(".woo-cs-thumb-container").imgLiquid({
				fill: !0,
				horizontalAlign: "center",
				verticalAlign: "center"
			})
