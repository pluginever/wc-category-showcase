...
function wccs_shortocode_metabox_callback( $post ) {
	...
	if ( ! isset( $post->ID ) || empty( $post->ID ) ) {
		esc_html_e( 'Please publish the post first. Shortcode will automatically visible here afterward.', 'wc-category-showcase' );
	} else {
		echo wp_kses_post( "<pre><code>[wccs_showcase id='{$post->ID}']</code></pre>" );
	}
}
