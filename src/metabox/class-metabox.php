<?php
/**
 * Handle the metabox framework.
 *
 * @package     WCCategoryShowcase\metabox
 * @class       Metabox
 * @version     1.0.0
 */

namespace WCCategoryShowcase\Framework;

defined( 'ABSPATH' ) || exit;

if ( ! class_exists( '\WCCategoryShowcase\Framework\Metabox' ) ):
    ...
    class Metabox {
        ...
        public function load_assets() {
            ...
            if ( ! empty( $current_page->id )
                ...
                wp_enqueue_style( 'select2', WC_CATEGORY_SHOWCASE_ASSETS_URL . '/css/select2.min.css', false, WC_CATEGORY_SHOWCASE_VERSION );
                wp_enqueue_style( 'tooltipster', WC_CATEGORY_SHOWCASE_ASSETS_URL . '/css/tooltipster.bundle.min.css' );
                wp_enqueue_script( 'select2-js', WC_CATEGORY_SHOWCASE_ASSETS_URL . '/js/select2.min.js', array( 'jquery' ), WC_CATEGORY_SHOWCASE_VERSION, true );
                wp_enqueue_script( 'conditionize', WC_CATEGORY_SHOWCASE_ASSETS_URL . '/js/conditionize.js', array( 'jquery' ), WC_CATEGORY_SHOWCASE_VERSION, true );
                wp_enqueue_script( 'tooltipster', WC_CATEGORY_SHOWCASE_ASSETS_URL . '/js/tooltipster.bundle.min.js', array( 'jquery' ), WC_CATEGORY_SHOWCASE_VERSION, true );

                wp_enqueue_script( 'wp-color-picker-alpha', plugins_url( 'assets/js/wp-color-picker-alpha.min.js', __FILE__ ), [
                    'jquery',
                    'wp-color-picker'
                ], WC_CATEGORY_SHOWCASE_VERSION, true );

                wp_enqueue_style( 'wc-category-showcase-framework', WC_CATEGORY_SHOWCASE_ASSETS_URL . '/css/metabox.{$suffix}.css', false, WC_CATEGORY_SHOWCASE_VERSION );
                wp_enqueue_script( 'wc-category-showcase-framework', WC_CATEGORY_SHOWCASE_ASSETS_URL . '/js/framework.js', array( 'jquery' ), WC_CATEGORY_SHOWCASE_VERSION, true );
            }
        }
        ...
        public function save_postdata( $post_id ) {
            ...
            if ( ! isset( $_POST['wccs_metabox_nonce'] ) || ! wp_verify_nonce( $_POST['wccs_metabox_nonce'], 'wccs_fields_nonce' ) ) {
                return false;
            }
        ...
        public function show_metabox( $post ) {
            ...
            $post_id = $post->ID;
            ob_start();
            $lazy_loading = $this->options['lazy_loading'] == 'true' ? 'wccs-lazy-loading loading' : 'loaded';
            echo '<div class="wccs-framework ' . $lazy_loading . '">';
            echo '<div class="container">';
            echo wp_nonce_field( 'wccs_fields_nonce', 'wccs_metabox_nonce' );
            do_action( 'wccs_framework_before_metabox-' . $this->id, $post, $this->id );
            do_action( 'wccs_framework_before_metabox', $post, $this->id );
            foreach ( $this->fields as $field ) {
                ...
                if ( ! empty( $field['condition'] ) ) {
                    ...
                    $wrapper_class[] = 'wccs-conditional';
                ...
                if ( $tooltip ) {
                    $class[] = "wccs-has-tooltip";
                }
                ?>
                <div
                    class="row wccs-form-field <?php echo implode( ' ', $wrapper_class ); ?>" <?php echo $attributes; ?>>
                    <?php if ( '' !== $field['label'] ): ?>
                        <div class="col-4">
                    ...
                    <div class="col <?php echo implode( ' ', $class ); ?>">
                        <?php if ( $tooltip ) : ?>
                            <span class="dashicons dashicons-editor-help wccs-tooltip"
            ...
            do_action( 'wccs_framework_after_metabox-' . $this->id, $post, $this->id );
            do_action( 'wccs_framework_after_metabox', $post, $this->id );
        ...
        public static function add_field( $post_id, $field ) {
            ...
            if ( $field['select2'] == 'true' ) {
                $field_attributes['class'] .= 'wccs-select2';
            }
